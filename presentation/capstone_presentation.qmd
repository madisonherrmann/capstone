...
Now that we can successfully predict lapse probability, how can we implement it for clinical benefit? With the addition of geolocation features, we can gain insight into not only when an individual is at a higher risk of lapsing, but why. By retraining the model with additional geolocation, I improved the models contextual awareness, a key step that brought us closer to deplying a real-time risk prediciton system in clinical settings.  The model was trained using high throughput computing with the XGBOOST algorithm. 5 by 10 kfold cross validation was used, and the best configuration resulted in a median auroc of .85. The histogram feveals the disuvtuion of auroc across the 50 folds. The drop in performance is likely due to us dropping the hour-by-hour windows, and using 24 hour periods instead. 
:::
