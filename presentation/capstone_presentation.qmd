...
:::{.notes}
-Alcohol and Opioid use disorders are chronic, requiring lifelong management and continuous monitoring. In 2022, 48.7 million people aged 12 or older (or 17.3%) had a substance use disorder. Only about  1 in 4 who were classified as needing treatment in the past
year received it. 
there are many barriers to accessing continued care in the United States, including affordability and availability. 
...
:::{.notes}
Our aim is to implement a risk-monitoring system that can deliver real-time, tailored interventions, while mitigating barriers in accessibility. Machine learning algorithms and sensing methods provide the ability to deliver personalized support and recommendations within a stand-alone support system. 
:::
...
:::{.notes} 
 A previous model based solely on 4x daily EMA's using hour-by-hour windows predicted next-day lapse in AUD with an outstanding auROC of 0.91, far surpassing industry benchmarks. This means there is a 91% chance that the model will score a lapse higher than a no-lapse. - Lapse window widths
:::
...
:::{.notes}
Started from day 1, this participants' report of past use drove up their lapse probability, as seen by the blue line The lime green line shows future efficacy, which is confidence in not drinking. There is almost an inverse relation between the two, as past use increases usually coincide with decreases inh future effiacy. This makes sense because if someone is currently using, their confidence in abstaining would likely decrease. Around day 50, there is a noticle drop in past use, bringing it down to baseline, or zero. Furture ffeiacy dips belw zero indicating this is decreasing that indoivuals risk of lapse. This graoh shows how differen risk factors were the most important oj different days.
:::
...
:::{.notes}
In the beginning, This participant had little change in their risk features, with past use and future efficacy bringing down their probability of lapse. However, around day 35, past use, the blue line, drives up their lapse probability a lot. This increase coincides with the slight increase in the purple line, which indicates time spent at a risky location. This is a geolocation feature that provides insights into why this individual may have lapsed. Moreover, future efficacy, the yellow-green line jumps shortly after past use slightly decreases. There is a somewhat inverse relation between future efficacy and past use, as expected. That is, if a person is currently using, it is likely to decrease future confidence about abstinence. At any given day, the most important feature impacting lapse probability can change. The ability top use these insights to deliver tailored interventions that support recovery is the first step towards clinical implementation. 
:::
